...
x=[];
y=[];
for k=1:1:int16((ydim/2))
    for l=int16((11/20)*xdim):int16((14/25)*xdim)
     sigma(ydim-k,l)=1.3;
     Eps_mat(ydim-k,l)=43;
     mu_mat(ydim-k,l)=0;
     x(k)=l;
...
imagesc((1:1:xdim),((1:1:ydim)),log10(Power/1e-3),[-40 0]);colorbar;
hold on
plot(x,y,'r');
hold on
for n=1:1:time_tot
...
%     hold on;
    imagesc((1:xdim),((1:ydim)),log10(Power),[-30 0]);colorbar;
    colormap(jet);
    hold on
    plot(x,y,'r');
    hold on
